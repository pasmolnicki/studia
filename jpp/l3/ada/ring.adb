package body Ring is
   package body Generic_Ring is
      ...
      begin
         ...
         return (Val => Natural ((Long_Long_Integer (Left.Val) * Long_Long_Integer (Right.Val)) mod Long_Long_Integer (N)), Valid => True);
      end "*";
      ...
      begin
         ...
         return (Val => Natural ((Long_Long_Integer (Left.Val) * Long_Long_Integer (Inv)) mod Long_Long_Integer (N)), Valid => True);
      end "/";
      ...
      function Fast_Pow (Base : Natural; Exp : Natural; Modulus : Positive) return Natural is
         Result : Natural := 1;
         B : Natural := Base mod Modulus;
         E : Natural := Exp;
      begin
         while E > 0 loop
            if (E mod 2) = 1 then
               Result := Natural ((Long_Long_Integer (Result) * Long_Long_Integer (B)) mod Long_Long_Integer (Modulus));
            end if;
            E := E / 2;
            B := Natural ((Long_Long_Integer (B) * Long_Long_Integer (B)) mod Long_Long_Integer (Modulus));
         end loop;
         return Result;
      end Fast_Pow;
      ...
      begin
         ...
         return (Val => Inverse(Item.Val), Valid => True);
      end Inverse;
      ...
      begin
         ...
         return (Val => Fast_Pow(Item.Val, Exp, N), Valid => True);
      end Pow;
